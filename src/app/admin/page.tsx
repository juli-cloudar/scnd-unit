  ...
  const scrapeBulk = async () => {
    ...
    try {
      const quickRes = await fetch('/api/vinted', {
        ...
        body: JSON.stringify({ mode: 'bulk', profileUrl: memberUrl, quick: true }),
      });
...
function AddTab({ user, toast, onProductAdded }: { user: Employee | null, toast: (msg: string, type?: ToastType) => void, onProductAdded: () => void }) {
  ...
  const handleSave = () => {
    ...
    const newProduct = { name: formData.name, category: formData.category, price: formData.price.replace(/^€/, ''), size: formData.size || '–', condition: formData.condition, images: formData.images, vinted_url: formData.vinted_url, sold: false };
...
// ⭐⭐⭐ DAS IST DER WICHTIGSTE TEIL - DEFAULT EXPORT ⭐⭐⭐
