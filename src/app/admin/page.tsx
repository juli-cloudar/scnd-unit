...
function AddTab({ user, toast, onProductAdded }: { user: Employee | null, toast: (msg: string, type?: ToastType) => void, onProductAdded: () => void }) {
  ...
  const scrapeVinted = async () => {
    ...
    try {
      ...
      if (existing) {
        toast(`⚠️ Artikel existiert bereits: "${existing.name}" (ID: ${existing.id})`, 'error');
        setIsScraping(false);
        return;
      }
  ...
  const handleSave = async () => {
    ...
    const { data: existing } = await supabase
      .from('products')
      .select('id, name')
      .eq('vinted_url', formData.vinted_url)
      .maybeSingle();
    
    if (existing) {
      toast(`❌ Artikel existiert bereits! (ID: ${existing.id})`, 'error');
      return;
    }
