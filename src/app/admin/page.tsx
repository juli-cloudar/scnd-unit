...
import { Trash2, ExternalLink, RefreshCw, ShoppingBag, Plus, Check, X, Edit3, Wand2, ChevronLeft, ChevronRight, ImageIcon, Lock } from 'lucide-react';
...
const ADMIN_PASSWORD = 'scndunit2025';
    ...
    <div className="min-h-screen bg-[#0A0A0A] flex items-center justify-center">
      <div className="w-full max-w-sm px-6">
        <div className="text-center mb-8">
          <h1 className="text-3xl font-bold tracking-tighter mb-1"><span className="text-[#FF4400]">SCND</span>_UNIT</h1>
          <p className="text-gray-500 text-xs uppercase tracking-widest">Admin Access</p>
        </div>
        <div className={`border p-6 transition-colors ${error ? 'border-red-500/50 bg-red-950/20' : 'border-[#FF4400]/20 bg-[#111]'}`}>
          <div className="flex items-center gap-2 mb-4 text-[#FF4400]">
            <Lock className="w-4 h-4" />
            <span className="text-xs uppercase tracking-widest font-bold">Passwort</span>
          </div>
          <input
            type="password"
            value={input}
            ...
            placeholder="••••••••••••"
            autoFocus
            className="w-full bg-[#1A1A1A] border border-[#FF4400]/30 px-4 py-3 text-sm text-[#F5F5F5] mb-3 focus:outline-none focus:border-[#FF4400]"
          />
          {error && <p className="text-red-400 text-xs mb-3 uppercase tracking-widest">Falsches Passwort</p>}
          <button
            onClick={tryLogin}
...
function AdminDashboard() {
  ...
  const openProductImages = (product: Product) => { setSelectedProduct(product); setSelectedImageIndex(0); };
  ...
  const scrapeVinted = async () => {
    if (!url.includes('vinted.de') && !url.includes('vinted.com')) { alert('Bitte gültige Vinted URL eingeben'); return; }
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setFormData(prev => ({ ...prev, vinted_url: url, name: '', price: '', size: '', images: [] }));
        setIsScraping(false); return;
      }
      if (data.images && data.images.length > 0) {
        ...
        setScrapedData(data); fillFormWithData(data, url);
        alert('Nur Teil-Daten gefunden. Bitte Bilder manuell hinzufügen.');
      } else { alert('Keine Daten gefunden. Bitte manuell eingeben.'); }
    } catch { alert('Netzwerkfehler. Bitte manuell eingeben.'); setFormData(prev => ({ ...prev, vinted_url: url })); }
  ...
  const pasteImageFromClipboard = async () => {
    try {
      ...
      } else { alert('Keine gültige Vinted-URL in Zwischenablage.'); }
    } catch { alert('Zugriff auf Zwischenablage verweigert.'); }
  };
  ...
  const extractFromUrlManual = () => {
    ...
    if (match) { window.open(`https://www.vinted.de/items/${match[1]}`, '_blank'); alert('Artikel geöffnet. Kopiere Bild-URLs manuell.'); }
    else { window.open(url, '_blank'); }
  };

  const cleanImageUrl = (url: string): string => url.replace(/[?&]s=[^&]+/, '').replace(/\/f\d+\//, '/f800/').replace(/\\/g, '').trim();

  const fillFormWithData = (data: ScrapedData, url: string) => {
    setFormData({ name: data.name || '', category: data.category || 'Jacken', price: data.price || '', size: data.size || '', condition: data.condition || 'Gut', vinted_url: url, images: data.images || [] });
  };

  const addImageUrl = () => {
    const newUrl = prompt('Vollständige Bild-URL eingeben:');
    if (newUrl && newUrl.includes('vinted.net')) { const cleaned = cleanImageUrl(newUrl); setFormData(prev => ({ ...prev, images: [...prev.images, cleaned] })); setCurrentImageIndex(formData.images.length); }
...
const newProduct = { 
  ...
  price: formData.price.startsWith('€') ? formData.price : `€${formData.price}`, // € hinzufügen falls nicht vorhanden
  size: formData.size || '–', 
  ...
  sold: false 
    ...
    if (!error) {
      ...
      showSuccess(`Produkt gespeichert!`);
  ...
  const removeProduct = async (id: number) => {
    const confirmed = await confirm({ title: 'Produkt löschen', message: 'Möchtest du dieses Produkt wirklich unwiderruflich löschen?', confirmText: 'Löschen', cancelText: 'Abbrechen', type: 'danger' });
    if (!confirmed) return;
    ...
    <div className="min-h-screen bg-[#0A0A0A] text-[#F5F5F5] font-sans">
              ...
              {selectedProduct.images.length > 1 && (
                <>
                  ...
                  <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-2">
                    {selectedProduct.images.map((_, i) => (<button key={i} onClick={() => setSelectedImageIndex(i)} className={`w-3 h-3 rounded-full transition-colors ${i === selectedImageIndex ? 'bg-[#FF4400]' : 'bg-white/50'}`} />))}
                  </div>
                </>
              )}
            </div>
      ...
      <div className="border-b border-[#FF4400]/30 px-6 py-4 flex items-center justify-between sticky top-0 bg-[#0A0A0A] z-10">
        ...
        <div className="flex gap-2 items-center">
          ...
          <button onClick={logout} className="px-4 py-2 text-xs uppercase tracking-widest font-bold border border-gray-700 text-gray-400 hover:text-red-400 hover:border-red-500/30 transition-colors">Logout</button>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto p-6">
        {activeTab === 'inventory' && (
          <div>
            <div className="flex items-center justify-between mb-6 gap-4 flex-wrap">
              <div className="flex gap-2 flex-wrap">{categories.map(cat => (<button key={cat} onClick={() => setFilter(cat)} className={`px-3 py-1.5 text-xs uppercase tracking-widest font-bold transition-colors ${filter===cat?'bg-[#FF4400] text-white':'border border-[#FF4400]/30 text-gray-400 hover:text-[#FF4400]'}`}>{cat}</button>))}</div>
                ...
                {filtered.map(p => (
                  <div key={p.id} className={`relative border transition-all ${p.sold?'border-red-500/50 opacity-50':'border-[#FF4400]/20 hover:border-[#FF4400]/50'}`}>
                    ...
                    <div className="aspect-[3/4] bg-[#1A1A1A] overflow-hidden relative group cursor-pointer" onClick={() => openProductImages(p)}>
                      ...
                      {p.images && p.images.length > 1 && (<div className="absolute bottom-2 right-2 bg-black/80 px-2 py-1 text-xs flex items-center gap-1 font-bold"><ImageIcon className="w-3 h-3" /> {p.images.length}</div>)}
                      <div className="absolute inset-0 bg-black/0 group-hover:bg-black/20 transition-colors flex items-center justify-center opacity-0 group-hover:opacity-100"><span className="bg-[#FF4400] text-white text-xs px-3 py-1 font-bold uppercase tracking-widest">Bilder ansehen</span></div>
                    </div>
                    <div className="p-2 bg-[#0A0A0A]">
                      ...
                      <div className="flex gap-1 mt-2">
                        <button onClick={() => markSold(p.id, p.sold)} className={`flex-1 py-1 text-xs font-bold uppercase tracking-widest transition-colors flex items-center justify-center gap-1 ${p.sold?'bg-green-600/20 text-green-400 hover:bg-green-600/30':'bg-red-600/20 text-red-400 hover:bg-red-600/30'}`}><ShoppingBag className="w-3 h-3" />{p.sold?'Reaktiv.':'Verkauft'}</button>
        ...
        {activeTab === 'add' && (
          <div className="max-w-4xl mx-auto">
            <h2 className="text-xl font-bold uppercase tracking-tight mb-6 flex items-center gap-2"><Plus className="w-5 h-5 text-[#FF4400]" /> Neues Produkt</h2>
            <div className="bg-[#111] border border-[#FF4400]/20 p-4 mb-4">
              <label className="text-xs uppercase tracking-widest text-[#FF4400] block mb-2 flex items-center gap-2"><Wand2 className="w-3 h-3" /> Schritt 1: Vinted URL</label>
              <div className="flex gap-2">
                <input value={url} onChange={e => setUrl(e.target.value)} placeholder="https://www.vinted.de/items/..." className="flex-1 bg-[#1A1A1A] border border-[#FF4400]/30 px-4 py-3 text-sm text-[#F5F5F5]" />
                <button onClick={scrapeVinted} disabled={isScraping || !url} className="px-6 py-3 bg-[#FF4400] text-white text-xs font-bold uppercase disabled:opacity-50">{isScraping ? '...' : 'Auto-Fill'}</button>
              </div>
            </div>
            <div className="bg-[#1A1A1A] border border-gray-700/50 p-4 mb-6">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                <button onClick={pasteImageFromClipboard} className="p-3 bg-[#252525] border border-gray-700 hover:border-[#FF4400]/50 text-left"><div className="text-[#FF4400] text-xs font-bold">📋 Aus Zwischenablage</div></button>
                <button onClick={pasteMultipleImages} className="p-3 bg-[#252525] border border-gray-700 hover:border-[#FF4400]/50 text-left"><div className="text-[#FF4400] text-xs font-bold">🖼️ Mehrere Bilder</div></button>
                <button onClick={() => addImageUrl()} className="p-3 bg-[#252525] border border-gray-700 hover:border-[#FF4400]/50 text-left"><div className="text-[#FF4400] text-xs font-bold">➕ Einzelnes Bild</div></button>
                <button onClick={extractFromUrlManual} className="p-3 bg-[#252525] border border-gray-700 hover:border-[#FF4400]/50 text-left"><div className="text-[#FF4400] text-xs font-bold">🔗 Vinted öffnen</div></button>
              </div>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="bg-[#111] border border-[#FF4400]/20 p-4">
                <div className="flex items-center justify-between mb-3">
                  <label className="text-xs uppercase tracking-widest text-[#FF4400]">Bilder ({formData.images.length})</label>
                  <div className="flex gap-2">
              ...
              <div className="bg-[#111] border border-[#FF4400]/20 p-4 space-y-4">
                ...
                <button onClick={addProduct} disabled={!formData.name || !formData.price || formData.images.length === 0} className="w-full py-4 bg-[#FF4400] text-white text-sm font-bold uppercase disabled:opacity-50">Speichern</button>
              </div>
            </div>
          </div>
        )}
