  ...
  try {
    ...
    for (const imgUrl of imageUrls.slice(0, 5)) {
      try {
        ...
        // Prüfen ob es ein gültiges Bild ist (mindestens 1KB)
        if (arrayBuffer.byteLength < 1000) continue;

        const contentType = imgResponse.headers.get('content-type') || 'image/webp';
        const base64 = Buffer.from(arrayBuffer).toString('base64');
        
        imagesBase64.push(`data:${contentType};base64,${base64}`);
      } catch (e) {
        console.error('Bild-Ladefehler:', e);
        continue;
      }
    }
    ...
    return NextResponse.json({
      ...
      condition,
      category,
      images: imagesBase64, // Base64 statt URLs!
    }, { headers: CORS });
