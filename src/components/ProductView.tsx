    ...
    return (
      <div className="space-y-3">
        {products.map((product) => (
          ...
          >
            <div className="w-20 h-20 md:w-24 md:h-24 shrink-0 bg-[#0A0A0A] overflow-hidden">
    ...
    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 xl:grid-cols-8 gap-1 md:gap-2">
      {products.map((product) => (
        <div 
          key={product.id} 
          onClick={() => window.location.href = `/products/${product.id}`}
          className="bg-[#1A1A1A] p-1 md:p-2 hover:ring-1 hover:ring-[#FF4400] transition-all text-center cursor-pointer group"
        >
          <div className="aspect-square bg-[#0A0A0A] overflow-hidden mb-1">
            <img src={product.images?.[0]} alt={product.name} className="w-full h-full object-cover" />
          </div>
          <p className="text-[10px] md:text-xs font-bold truncate group-hover:text-[#FF4400] transition-colors">{product.name}</p>
          <p className="text-[10px] md:text-xs text-[#FF4400]">{formatPrice(product.price)}€</p>
        </div>
      ))}
    </div>
  );
